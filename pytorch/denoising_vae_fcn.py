...
class VAE(torch.nn.Module):
    ...
    def encoder(self, input):
        noise = Variable(torch.from_numpy(np.random.normal(0, 1, size=input.size())).float().cuda())
        input_noise = noise*self.noise_factor + input
        return F.relu(self.encoder_linear1(input_noise))
