...
class CVAE(object):
    def __init__(self, sample_size, latent_size):
        ...
        self.sample_size = sample_size + 10
        self.output_size = sample_size
    ...
    def encoder(self):
        with tf.variable_scope('encoder'):
            ...
            net = tf.nn.xw_plus_b(self.input, w1, b1, name='encoder_layer1')
            net = tf.nn.relu(net)
    ...
    def decoder(self, z):
        z = tf.concat([z, self.label], 1)
        w1 = tf.get_variable('decoder_w1', shape=[self.latent_size + 10, self.dh_dim], initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable('decoder_b1', shape=[self.dh_dim], initializer=tf.constant_initializer(0))
        w2 = tf.get_variable('decoder_w2', shape=[self.dh_dim, self.output_size], initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable('decoder_b2', shape=[self.output_size], initializer=tf.constant_initializer(0))
    ...
    def loss(self, logits, z_mu, z_var):
        recon_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=self.input_), 1)
